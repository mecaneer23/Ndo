...
class Styles:
    ...
    def _get_subscript(self, char: str) -> str:
        """
        Convert a standard character to a subscript character
        """
        chars: dict[str, str] = {
            "0": "₀",
            "1": "₁",
        ...
        if char not in chars:
            msg = f"Character '{char}' cannot be converted to subscript"
            raise ValueError(msg)
        return chars[char]

    def _handle_subscripts(self) -> None:
        """
        Convert characters between a `_` and a ` ` to subscript characters
        """
        msg = "Subscript handling not implemented yet"
